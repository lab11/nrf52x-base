...
TfLiteStatus MicroInterpreter::Invoke() {
  ...
  // error_reporter_->Report("Starting Registration Init!");

  // Init method is not yet implemented.
  for (size_t i = 0; i < operators_->size()-2; ++i) {
    auto* node = &(node_and_registrations_[i].node);
    auto* registration = node_and_registrations_[i].registration;

    // error_reporter_->Report("Operation: %s", OpNameFromRegistration(registration));
  ...
  // error_reporter_->Report("Starting Registration Prepare!");
  ...
  // error_reporter_->Report("Starting Registration Invoke!");
  ...
  for (size_t i = 0; i < operators_->size()-2; ++i) {
    ...
    // TfLiteTensor* output = GetOutput(&context_, node, 0);
    // TfLiteTensor* input = GetInput(&context_, node, 0);
    // char * input_name = input->name;
    // char * output_name = output->name;

    // error_reporter_->Report("Input name: ", input->name);
    // error_reporter_->Report("Operation: %s", OpNameFromRegistration(registration));
    // error_reporter_->Report("Node Info");
    // error_reporter_->Report("Node Size %d", node->inputs->size);
    // for (int i = 0; i < node->inputs->size; i++){
    //   error_reporter_->Report("Input: %d", node->inputs->data[i]);
    // }

    // for (int i = 0; i < node->outputs->size; i++){
    //   error_reporter_->Report("Outputs: %d", node->outputs->data[i]);
    // }
    // error_reporter_->Report("Output name: ", output->name);
