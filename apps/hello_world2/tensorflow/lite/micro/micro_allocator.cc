...
TfLiteStatus AllocateVariables(
  ...
  //CHANGED tensor from size in
  for (size_t i = 0; i < flatbuffer_tensors->size()-2; ++i) {
...
AllocationInfo* AllocateAndCalculateAllocationInfo(
  ...
  for (size_t i = 0; i < subgraph->inputs()->size(); ++i) {
    const int tensor_index = 1; //Setting input index to 1 which corresponds to input_1_int8
    AllocationInfo* current = &allocation_info[tensor_index];
    current->first_created = 0;
  }
  ...
  // }
  for (size_t i = 0; i < subgraph->outputs()->size(); ++i) {
    const int tensor_index = 0;
    AllocationInfo* current = &allocation_info[tensor_index];
    current->last_used = subgraph->operators()->size() - 2 - 1;
  }
  ...
  for (int i = (subgraph->operators()->size() - 2); i >= 1; --i) {
    const auto* op = subgraph->operators()->Get(i);
    for (size_t n = 0; n < op->inputs()->size(); ++n) {
      const int tensor_index = op->inputs()->Get(n);
      AllocationInfo* current = &allocation_info[tensor_index];
...
TfLiteStatus MicroAllocator::Init() {
  ...
  //CHANGED to size -1 to account for tensor
  context_->tensors_size = tensors_->size() - 2;
  ...
  size_t pos = 0;
  size_t i = 0;
  for (i; i < tensors_->size(); ++i) {
    const char * name = tensors_->Get(i)->name()->c_str();
    // error_reporter_->Report("Tensor pos %d i %d name is %s", pos, i, name);
    //CHANGED to omit initialization of "input_1"
    if(strcmp(name, input_one) != 0 && strcmp(name, Identity) != 0){
      TfLiteStatus status = internal::InitializeRuntimeTensor(
      memory_allocator_, *tensors_->Get(i), model_->buffers(),
      error_reporter_, &context_->tensors[pos]);
  ...
  // error_reporter_->Report("Final i: %d Final pos: %d ", i, pos);
...
TfLiteStatus MicroAllocator::AllocateNodeAndRegistrations(
  ...
  auto* output = reinterpret_cast<NodeAndRegistration*>(
    memory_allocator_->AllocateFromTail(
        sizeof(NodeAndRegistration) * operators_->size()-2,
        alignof(NodeAndRegistration)));
  if (output == nullptr) {
    error_reporter_->Report(
  ...
  int pos = 0;
  //CHANGED to 1 from 0
  for (size_t i = 1; i < operators_->size()-1; ++i) {
    ...
    const auto* registration = output[pos].registration;

    // error_reporter_->Report("Operator %d is %s", i, OpNameFromRegistration(registration));
    ...
    TfLiteNode* node = &(output[pos].node);
    *node = {};
    node->inputs = inputs_array;
    node->outputs = outputs_array;

    // if(strcmp(OpNameFromRegistration(registration), "DEQUANTIZE") == 0){
    //   error_reporter_->Report("Entered output is %d",outputs_array->data[0]);
    //   // TfLiteIntArray arr;
    //   // arr.size = 1;
    //   // int data [1] = { 86 };
    //   // arr.data = data;
    //   // node->outputs = &arr;
    //   outputs_array->data[0]= 86;
    //   error_reporter_->Report("Entered output is %d",outputs_array->data[0]);
    ...
    //   error_reporter_->Report("%d",inputs_array->data[i]);
    // }

    // error_reporter_->Report("Outputs: ");
    // for (int i = 0; i < outputs_array->size; i++){
    //   error_reporter_->Report("%d",outputs_array->data[i]);
    // }
...
TfLiteStatus MicroAllocator::FinishTensorAllocation() {
  ...
  // but is thrown away afterwards.
  {
    ...
    size_t allocation_info_size = tensors_->size()-2;
    ...
    // error_reporter_->Report("Maximum Memory Used %d", planner.GetMaximumMemorySize());
    // error_reporter_->Report("Arena Size: %d", arena_size);
    // error_reporter_->Report("Memory Size: %d", memory_allocator_->GetDataSize());
